	...
	if(!ContinueActive(user, feed_target))
		if(!passive_feed)
			...
			if(ishuman(feed_target))
				...
				feed_target.add_blood_DNA_to_items(feed_target.get_blood_dna_list(), ITEM_SLOT_ICLOTHING|ITEM_SLOT_OCLOTHING)
			feed_target.add_splatter_floor(get_turf(feed_target))
			if(ishuman(user))
				user.add_blood_DNA_to_items(feed_target.get_blood_dna_list(), ITEM_SLOT_ICLOTHING|ITEM_SLOT_OCLOTHING)
