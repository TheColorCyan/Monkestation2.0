...
/obj/machinery/rnd/proc/process_item(obj/item/item_inserted, list/mats_consumed, amount_inserted)
	...
	if(use_energy(ROUND_UP((amount_inserted / (MAX_STACK_SIZE * SHEET_MATERIAL_AMOUNT)) * 0.01 * initial(active_power_usage))))
		var/mat_name = "iron"
...
/obj/machinery/rnd/production/ui_act(action, list/params, datum/tgui/ui)
	...
	switch (action)
		if("remove_mat")
			...
			//we use initial(active_power_usage) because higher tier parts will have higher active usage but we have no benifit from it
			if(!use_energy(ROUND_UP((amount / MAX_STACK_SIZE) * 0.01 * initial(active_power_usage))))
