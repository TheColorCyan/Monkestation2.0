...
/obj/projectile/proc/check_ricochet_flag(atom/target)
	...
	if(expanded_bounce && (target.flags_ricochet & (RICOCHET_HARD | RICOCHET_SHINY)))
		return TRUE
...
/obj/projectile/proc/generate_hitscan_tracers(cleanup = TRUE, duration = 3, impacting = TRUE)
	...
	if(tracer_type)
		...
		for(var/datum/point/beam_point in beam_segments)
			generate_tracer_between_points(beam_point, beam_segments[beam_point], tracer_type, color, duration, hitscan_light_outer_range, hitscan_light_color_override, hitscan_light_intensity, tempref)
